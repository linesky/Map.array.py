...
images=None
ccanvas=None
def convert_16bit_to_rgb(high_byte, low_byte):
    # Extrai os bits relevantes do high byte e low byte
    red = (high_byte & 0b0000111100000000) >> 4
    green_low = (low_byte & 0b00110000) >> 4
    green_high = (high_byte & 0b0000000000110000) << 2
    green = green_high | green_low
    blue = (low_byte & 0b00001111) << 4
...
def msgbox(msgs:str,color:str):
    ...
    for y in range(Height):
        for x in range(Width):
            ...
            rgb= f1.read(2)
            r,g,b=convert_16bit_to_rgb(rgb[0],rgb[1])
